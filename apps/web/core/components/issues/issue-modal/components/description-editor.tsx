...
export const IssueDescriptionEditor = observer(function IssueDescriptionEditor(props: TIssueDescriptionEditorProps) {
  ...
  const handleAiAssistance = async (response: string) => {
    ...
    editorRef.current?.setMarkdownValueAtCursorPosition(response);
  };
  ...
  const handleAutoGenerateDescription = async () => {
    ...
    aiService
      .createGptTask(workspaceSlug.toString(), {
        prompt: issueName,
        task: "Gere uma descrição completa para este item de trabalho.",
      })
      .then((res) => {
        if (res.response === "")
          setToast({
            ...
            title: "Erro!",
            message:
              "O título do item de trabalho não é descritivo o suficiente para gerar a descrição. Tente outro título.",
          });
        else handleAiAssistance(res.response);
      })
      .catch((err) => {
        ...
        if (err.status === 429)
          setToast({
            type: TOAST_TYPE.ERROR,
            title: "Erro!",
            message: error || "Você atingiu o limite de 50 solicitações por mês por usuário.",
          });
        else
          setToast({
            type: TOAST_TYPE.ERROR,
        ...
        <>
          ...
          <div className="border-0.5 z-10 flex items-center justify-end gap-2 p-3">
            {issueName && issueName.trim() !== "" && config?.has_llm_configured && (
              <button
                type="button"
                className={`flex items-center gap-1 rounded-sm bg-surface-2 px-1.5 py-1 text-caption-sm-regular hover:bg-layer-1 ${iAmFeelingLucky ? "cursor-wait" : ""
                  }`}
