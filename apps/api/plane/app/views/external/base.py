...
def get_llm_config() -> Tuple[str | None, str | None, str | None, str | None]:
    ...
    api_key, provider_key, model, api_base = get_configuration_value(
        [
            ...
            {
                "key": "LLM_API_BASE",
                "default": os.environ.get("LLM_API_BASE", os.environ.get("OPENAI_API_BASE", None)),
            },
        ]
    )
    ...
    # Validate model is supported by provider. For OpenAI-compatible endpoints,
    # allow custom model names configured by the user.
    if provider_key.lower() != "openai" and model not in provider.models:
        log_exception(
            ValueError(
